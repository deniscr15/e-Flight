			...
			<div class="card-body py-4">
				...
				<div class="row mb-3">
					...
					<div class="col-md-12 pe-lg-10">
						<!--begin::Form-->
						<form enctype="multipart/form-data" action="{{route('transactions.update', $transaction->id)}}" method="POST" class="form mb-15" method="post" id="kt_contact_form">
							@csrf
							@method('PUT')
							...
							<div class="row mb-5">
								<div class="col-md-12 fv-row">
									...
									<input id="date" type="date" class="form-control form-control-solid" placeholder="" name="date" value="{{$transaction->date}}" required/>
								</div>
							</div>
                            ...
                            <div class="row mb-5">
                                <div class="col-md-12 fv-row">
                                    ...
                                    <select id="doctor" name="id_doctor" class="form-select form-select-solid" required>
										...
										<option value="{{$doctor->id}}" {{$doctor->id == $transaction->id_doctor ? 'selected' : ''}} >{{$doctor->name}}</option>
										@endforeach
                                    </select>
                                </div>
                            </div>
							...
							<div class="row mb-5">
                                <div class="col-md-12 fv-row">
                                    ...
                                    <select id="patient" name="id_patient" class="form-select form-select-solid" required>
										...
										<option value="{{$patient->id}}" {{$doctor->id == $transaction->id_patient ? 'selected' : ''}} >{{$patient->name}}</option>
										@endforeach                                     
                                    </select>
                                </div>
                            </div>
							...
							<div class="row mb-5">
                                <div class="col-md-12 fv-row">
                                    ...
                                    <select id="schedule" name="schedule" class="form-select form-select-solid disable" required>
                                        <option value="{{$selected_schedule['id']}}"> {{$selected_schedule['start_time']}} - {{$selected_schedule['end_time']}} </option>                                          
                                    </select>
                                </div>
                            </div>
							...
							<div class="row mb-5">
                                <div class="col-md-12 fv-row">
                                    ...
                                    <select id="status" name="status" class="form-select form-select-solid" required>
										...
										<option value="soon" {{$transaction->status == 'soon' ? 'selected' : ''}} >Soon</option>
										<option value="started" {{$transaction->status == 'started' ? 'selected' : ''}} >Started</option>
